//정수 n이 주어질 때, n이하의 짝수를 모두 더한 값을 return 하도록 solution 함수를 작성해주세요.https://school.programmers.co.kr/learn/courses/30/lessons/120831
function solution(n) {
  return;
}
// _ 를 사용하면 앞으로 안 쓸 함수
...
Array(10)
  .fill(0)
  .map((_, i) => i + 1)
  .filter((v) => v % 2 === 0).reduce;
Array(10)
  .fill(0)
  .map((_, i) => i + 1)
  .filter((v) => v % 2 === 0)
  //   .reduce((a, c) => a + c); // XXXXXXX
  .reduce((a, c) => a + c, 0); // ★★★★★★
// 30
...
let result = 0;
let data = [149, 180, 192, 170].forEach((v) => {
  v > 168 ? result++ : null;
});
result;
...
// join에서 주의사항
[010, 5044, 2903]
  .join("") // 010은 8진법으로 8이 나온다 011은 9로 나온다.
  [("010", "5044", "2903")].join("");
// 보통 이렇게 문자로 처리하기 때문에 문제가 생기지 않는다
// 0으로 시작하면 앞에 0이 사라진다.
...
// 문제 4
// 1부터 100까지의 숫자 중 3의 배수만 더한 값을 출력해라
// 앞에서는 forEach 사용했는데 , 이번에는 map과 filter, reduce를 사용해라ㅡ

// map을 사용해서 1부터 100까지 중 3의 배수만 더한 값
...
// 문제 5
// 1부터 100까지의 숫자 중 1이라는 숫자가 들어간 숫자는 모두 더해라
//ex 더하는 숫자 : 1 , 10 ,11, 12 , 13 ......
//ex 더하지 않는 숫자 : 2,3,4,5,6,7,8,9,20....

Array(100)
  .fill(0)
  .map((_, i) => i + 1)
  .filter((v) => v.toString().includes('1')).reduce((a,c)=>a+c,0)
